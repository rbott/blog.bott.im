...
I would not argue that QEMU is "bloated" in a negative way (as it has proven quite stable over the past years), but it does combine _a lot_ of different usecases in one single codebase. This also means that you need to install quite a lot of dependencies on your host if you _just_ want to spawn a few VMs and also increases the attack surface if you operate in an environment with increased security restrictions. And last but not least, it does not make QEMU exceptionally fast in terms of initialisiation speeds.

The latter has led to the creation of other projects like [Firecracker](https://firecracker-microvm.github.io/) which aim at being a minimalist virtual machine manager with only a fraction of the features QEMU sports. They are especially well suited if containers do not provide the flexibility / isolation your environment needs, but at the same time exceptionally fast boot times and a greatly reduced attack surface are a requirement.
...
Let's start with a quick introduction into QEMU machine types. A machine type describes the system QEMU emulates. You can query your QEMU installation for a list of supported machine types (output truncated for readability):
...
Now what happens if you spawn a VM with the `microvm` machine type? This will leave you with a _very_ stripped down and simple VM/guest environment:

- no PCI or PCIe bus
- no USB

With that said, you will _only_ be able to use a very limited set of VirtIO devices which use the simple `virtio-mmio` interface instead of a PCI bus:
...
- emulation of "classic" NICs or disk controllers (as these are usually ISA or PCI devices)
...
That's down to almost a quarter of the libraries compared to the full build! Unfortunatly, Debian does not ship (yet?) a separate package for the `microvm` binary, so you will still end up with a bunch of extra libraries installed, but at least they are not used at runtime.
...
/usr/bin/qemu-system-x86_64-microvm \
 ...
 -chardev socket,path=/tmp/microvm-test.serial,server=on,wait=off,id=char0 \
...
/usr/bin/qemu-system-x86_64-microvm \
 ...
 -device virtconsole,chardev=char0,name=console0 \
 -chardev socket,path=/tmp/microvm-test.serial,server=on,wait=off,id=char0 \
 -netdev type=tap,id=net-dev-0,ifname=tap0 \
 -device virtio-net-device,netdev=net-dev-0,mac=aa:00:00:c7:87:b0 \
 -blockdev driver=raw,node-name=disk-dev-0,file.driver=host_device,file.filename=/dev/vg0/microvm_test \
 -device virtio-blk-device,id=disk-dev-0,drive=disk-dev-0 \
 -nodefaults -no-user-config -nographic
 ```